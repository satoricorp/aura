...
interface Splat {
  ...
  dx: number;
  dy: number;
  /** Pointer splats use >1 so interaction reads stronger than idle stirring */
  amp: number;
}

/** Keeps the velocity field moving when idle (no pointer) — drives slow paint swirl */
function idleVelocitySplats(t: number): Splat[] {
  const out: Splat[] = [];
  for (let i = 0; i < 5; i++) {
    const phase = (i * Math.PI * 2) / 5;
    const cx = 0.5 + 0.34 * Math.cos(t * 0.12 + phase);
    const cy = 0.5 + 0.34 * Math.sin(t * 0.1 + phase * 1.2);
    out.push({
      x: cx,
      y: cy,
      dx: Math.sin(t * 0.35 + i * 1.1) * 12,
      dy: Math.cos(t * 0.32 + i * 1.0) * 12,
      amp: 1,
    });
  }
  return out;
...
export function useFluid({
  ...
  useFrame((state) => {
    ...
    const currentSplats = [
      ...idleVelocitySplats(t),
      ...splatsRef.current,
    ];
    splatsRef.current = [];

    // Velocity splats only (color comes from warped UV sampling in the logo shader, not dye)
